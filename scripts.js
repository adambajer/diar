        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            setupAnnyang(); // Initialize Annyang for voice input
            setupListeningIndicator(); // Update mic indicator based on Annyang's state
        });
        ...
        function updateYearAndMonthDisplay() {
            ...
            if (selectedMonthElement) {
                const currentMonthName = baseDate.toLocaleString('cs-CZ', { month: 'long' });
                selectedMonthElement.innerText = currentMonthName;
            } else {
        ...
        // ========================
        // Annyang Integration for Voice Input
        // ========================
        ...
        function setupAnnyang() {
            if (annyang) {
                ...
                const commands = {
                    'add note to day *day at hour *hour :text': function(day, hour, text) {
                        addVoiceNote(day, hour, text);
                    },
                    'edit note to day *day at hour *hour :text': function(day, hour, text) {
                        addVoiceNote(day, hour, text);
                    },
                    'delete note to day *day at hour *hour': function(day, hour) {
                        deleteVoiceNote(day, hour);
                    },
                    'clear note to day *day at hour *hour': function(day, hour) {
                        deleteVoiceNote(day, hour);
                    }
                };
                ...
                // Initialize microphone indicator
                updateMicIndicator(annyang.isListening());

                // Optional: Listen for microphone toggle clicks
        ...
        // Function to add or edit a note via voice
        function addVoiceNote(dayStr, hourStr, text) {
            const dayIndex = parseDayStringToIndex(dayStr);
            const hourIndex = parseHourStringToNumber(hourStr);

            if (dayIndex === null || hourIndex === null) {
                alert("Neplatný den nebo hodina. Použijte například: 'Pondělí' pro den a '7' pro hodinu.");
                return;
            }
            ...
            if (cell) {
                cell.innerText = text;
                const dateObj = addDays(currentStartOfWeek, dayIndex);
                const date = format(dateObj, 'yyyy-MM-dd');
                const time = formatHour(hourIndex);
                saveNoteToFirebase(date, time, text);
                saveSelectedDateToLocalStorage(dateObj); // Save to local storage
                alert(`Poznámka přidána/aktualizována pro den ${dayStr} v hodině ${hourStr}.`);
            } else {
                alert("Příslušná buňka nebyla nalezena.");
            }
        }

        // Function to delete a note via voice
        function deleteVoiceNote(dayStr, hourStr) {
            const dayIndex = parseDayStringToIndex(dayStr);
            const hourIndex = parseHourStringToNumber(hourStr);
            ...
            if (cell) {
                ...
                deleteNoteFromFirebase(date, time);
                saveSelectedDateToLocalStorage(dateObj); // Save to local storage
                alert(`Poznámka smazána pro den ${dayStr} v hodině ${hourStr}.`);
            } else {
                alert("Příslušná buňka nebyla nalezena.");
            }
        }

        // Helper function to parse day string to index
        function parseDayStringToIndex(dayStr) {
            const daysMap = {
                'pondělí': 0,
                'úterý': 1,
                'středa': 2,
            ...
            const normalizedDayStr = dayStr.toLowerCase();
            return daysMap[normalizedDayStr] !== undefined ? daysMap[normalizedDayStr] : null;
        }

        // Helper function to parse hour string to number
        function parseHourStringToNumber(hourStr) {
        ...
        function updateMicIndicator(isListening) {
            ...
            if (isListening) {
                micIndicator.innerHTML = `<i class="bi bi-mic-fill" style="font-size: 2rem; color: #0d6efd;"></i>`;
            } else {
                micIndicator.innerHTML = `<i class="bi bi-mic-fill" style="font-size: 2rem; color: #6c757d;"></i>`;
            }
        }
        ...
        function setupListeningIndicator() {
            // Initialize the microphone indicator based on Annyang's current state
            if (annyang) {
                updateMicIndicator(annyang.isListening());
            }
        } 